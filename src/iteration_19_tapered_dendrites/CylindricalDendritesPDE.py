...
simulation_params = CableParameters(c_m=1 * uF / cm ** 2,
                                    Rm=Rm,
                                    gL=1 / Rm,
                                    ra=100 * ohm * cm,
                                    L=500.0 * um,
...
class CylindricalDendriticTreePDECase(unittest.TestCase):
    ...
    def test_difussion_pde_triagonal_matrix(self, verbose=False, x_N=1001, t_max=1 * ms):
    ...
    def test_difussion_pde_triagonal_matrix_in_SI_units(self, verbose=False, x_N=1001, t_max=1 * ms):
        ...
        r_of_x = np.ones(N) * simulation_params.r0
        b = simulation_params.b
        assert have_same_dimensions(b, meter ** 2 / second)

        difussion = np.ones(N - 1) * b / simulation_params.dx ** 2
