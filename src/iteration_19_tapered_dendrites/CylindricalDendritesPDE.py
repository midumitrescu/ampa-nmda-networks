...
class CylindricalDendriticTreePDECase(unittest.TestCase):
    ...
    def test_difussion_pde_triagonal_matrix(self, verbose=False, x_N=301, t_max=20 * ms):
        ...
        def solve(x0, plot=True, t_max=300 * ms, dt=0.01 * ms):
            ...
            if plot:
                plot_difussion_unitless_split(times=times, x=x, r_of_x=r_of_x, V_s=V_s, dt=dt)
    ...
    def test_crank_nicolson_split_single(self, verbose=False, x_N=11, t_max=1 * ms):
        simulation_params = default_params.with_property(t=t_max, N=x_N)
        N = simulation_params.N
        si_units = simulation_params.to_numerical()
        ...
        def crank_nicolson(x0, t_span, V0, dt=to_SI(0.01 * ms), saved_frames=1, verbose=False):
            ...
            for step in range(1, num_steps + 1):
                ...
                # RHS
                rhs = R @ V + 1 / 2 * dt * (
                        synaptic_input_profile(t=t, x0=x0, dt=dt/2) + synaptic_input_profile(t=t + dt/2, x0=x0, dt=dt/2))
        ...
        def solve(x0, plot=True, t_max=to_SI(300 * ms), dt=to_SI(0.01 * ms)):
            ...
            if plot:
                plot_difussion_unitless(times=times, V_s=V_s, simulation_params=si_units, dt=dt,
                                        sim_type="Crank-Nicolson split and unitless")
    ...
    def test_difussion_pde_crank_nicolson_unitless(self, verbose=False, x_N=1001, t_max=to_SI(30 * ms)):
        ...
        x0_values = [to_SI(250 * um)]
        dts = [to_SI(1E-7 * second),  to_SI(0.5E-6 * second), to_SI(1E-6 * second)]
        calls = list(itertools.product(x0_values, dts))
