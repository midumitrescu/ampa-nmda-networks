'''
From https://github.com/babicknell/Dendrites
'''
import tqdm
...
def build_rate_seq_slow(rates, T0 =0 * second, T=1 * second):
    ...
    n_expected = int(max_rate * T * 3) + 10

    isi_s = np.random.exponential(
        scale=1 / (rates[:, None]),
        size=(len(rates), n_expected),
    )
