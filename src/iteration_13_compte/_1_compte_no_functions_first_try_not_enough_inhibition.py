...
def to_spike_trains(spikemon: SpikeMonitor, neurons: list[int]):
    neurons = np.array(neurons)  # e.g. [3, 7, 12, 20]
    dt_ms = spike_mon.clock.dt/ms
    T_ms = float(spikemon.t[-1] / ms)

    T = int(np.ceil(T_ms / dt_ms)) + 1
    N = len(neurons)
...
population_rate_mon = PopulationRateMonitor(E)
...
for i in range(NE):
    spikes = spike_mon.spike_trains()[i] / ms
...
neurons = [0, 50, 80]
raster = to_spike_trains(spike_mon, neurons=neurons)
rates = raster_to_rates(raster, spike_mon.clock.dt)

plt.plot(population_rate_mon.t / ms, population_rate_mon.smooth_rate(width=5 * ms), label="Population")
for neuron_index, rate in zip(neurons, rates):
    plt.plot(population_rate_mon.t / ms, rate, label=f"Neuron {neuron_index}")
plt.legend()
plt.show()