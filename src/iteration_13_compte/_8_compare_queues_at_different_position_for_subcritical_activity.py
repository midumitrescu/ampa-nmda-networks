...
from brian2 import nS, SpikeMonitor, PopulationRateMonitor, StateMonitor, ms, prefs, defaultclock, nF, mV, start_scope, \
    NeuronGroup, Synapses, PoissonInput, second, devices, run, network_operation, Hz

from iteration_13_compte.compte_utils import CueInfo, default_cue_info
...
def add_cue_input(degree, group: NeuronGroup, Jp, Jm, example: AnExampleExperiment, cue: CueInfo = default_cue_info):
    ...
    cue_w = Jm + (Jp - Jm) * np.exp(-(dtheta[mask] ** 2) / (2 * cue.sigma ** 2))
...
'''
    ...
    #cue_1 = CueInfo(delay=200 * ms, duration=200 * ms, sigma=2, spread=10, cue_rate=50 * Hz)
    cue_2 = CueInfo(delay=600 * ms, duration=400 * ms, sigma=4, spread=50, cue_rate=1000 * Hz)

    #bla_1, cue_controller_1 = add_cue_input(degree=90, group=E, Jp=Jp_EE, Jm=Jm, example=example, cue=cue_1)
    bla_2, cue_controller_2 = add_cue_input(degree=270, group=E, Jp=Jp_EE, Jm=Jm, example=example, cue=cue_2)
    ...
    #print(cue_controller_1)
    print(cue_controller_2)
    ...
    #plot_compte_results(results=compte_result, theta_array_assignment=theta_E, cues = [cue_1, cue_2])
