...
from brian2 import nS, SpikeMonitor, PopulationRateMonitor, StateMonitor, ms, msecond, prefs, defaultclock, nF, mV, Hz, \
    start_scope, NeuronGroup, Synapses, PoissonInput, second, devices, run, profiling_summary
...
class AnExampleExperiment:
    ...
    def __init__(self, G_EE_AMPA, G_EE_NMDA, G_EI, G_IE, G_II, NE, NI, label, NE_ref=2048, NI_ref=512, seed=None):
...
def compute_ordered_firing_rates(example: AnExampleExperiment, spikes_monitor: SpikeMonitor, runtime, theta_array_assignment):
...
def plot_compte(sim_time, population_rate_monitor: PopulationRateMonitor, spikes_monitor: SpikeMonitor, currents_monitor: StateMonitor, theta_array_assignment, example: AnExampleExperiment):
...
def execute_compte_experiment(example: AnExampleExperiment, seed: int = None):
    ...
    #index_50_degrees = int(50 * 800 / 360)
    #index_120_degrees = int(120 * 800 / 360)
    #plt.plot(theta_E, W_EE[index_50_degrees], label="50 degrees")
    #plt.plot(theta_E, W_EE[index_120_degrees], label="120 degrees")
    #plt.legend()
    #plt.show()
    ...
    mean_excitatory_input = tau_AMPA * 1000 * 1.8 * Hz * gext_E
    ...
    plot_compte(sim_time=runtime, population_rate_monitor=population_rate_monitor, spikes_monitor=spike_monitor, currents_monitor=currents_monitor, theta_array_assignment=theta_E, example=example)

    return population_rate_monitor


default_experiment = AnExampleExperiment(G_EE_AMPA=0, G_EE_NMDA=0.7, G_EI=0.292, G_IE=1.64, G_II=1, NE=800, NI=200, label="Experiment showing unstable high activity")

# Threshold:
#
#for g_EE in np.linspace(0.83, 9, 50):

results = []
for g_EE in np.linspace(0.82, 0.83, 11, endpoint=True):
    for seed in range(10):
        current = AnExampleExperiment(G_EE_AMPA=0, G_EE_NMDA=g_EE, G_EI=0.292, G_IE=1.64, G_II=1, NE=800, NI=200,
                            label=f"look for high activity {g_EE}?", seed=seed)
        rates_monitor = execute_compte_experiment(example=current, seed=None)
        rates = rates_monitor.smooth_rate(width=20 * ms) / Hz
        last_20_percent = int(0.8 * len(rates))
        results.append({
            "g_EE": g_EE,
            "seed": seed,
            "final_rate": np.mean(rates[last_20_percent:])
        })
...
..    ...   ...         ...
105  0.83     5    0.048277
106  0.83     6  499.118132
107  0.83     7  499.120464
108  0.83     8  499.084315
109  0.83     9  499.065523
