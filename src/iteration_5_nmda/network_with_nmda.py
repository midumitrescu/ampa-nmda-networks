...
wang_model_with_extra_variables = f""" {wang_model}
...
translated_model_with_extra_variables = f""" {translated_model}
I_nmda = g_nmda * sigmoid_v * (v-E_nmda): amp / meter**2
one_minus_g_nmda = 1- g_nmda/siemens * meter**2 : 1
"""
...
def plot_internal_states_(experiment, internal_states_monitor):
    fig, ax = plt.subplots(1, 1)

    for i in range(0, len(internal_states_monitor.sigmoid_v)):
        ax.plot(internal_states_monitor.t / ms, internal_states_monitor[i].sigmoid_v, label=f"sigmoid {i}")
    ax.set_xlabel("t [ms]")
    ax.set_ylabel("activation [unitless]")
