...
def sim_diffusion_process_for_up_state_(experiment: Experiment):
    ...
    simulation_results = sim_diffusion_process_for_one_state(experiment, state=experiment.network_params.up_state)
...
def sim_diffusion_process_for_one_state(experiment: Experiment, state: State) -> SimulationResults:
    start_scope()
    ...
    g_e_0 = state.effective_timeconstant_estimation.mean_excitatory_conductance()
    g_i_0 = state.effective_timeconstant_estimation.mean_inhibitory_conductance()
    x_0 = state.effective_timeconstant_estimation.mean_nmda_activation()
...
def sim_diffusion_process_with_up_down(experiment: Experiment) -> SimulationResults:
    start_scope()
    ...
    def toggle_inputs(t):
        ...
        if step % 2 == 0:
            diffusion_network.up = 1
            diffusion_network.down = 0
            logger.debug("at {} we have up=1 and down=0", t / ms)
        else:
            diffusion_network.up = 0
            diffusion_network.down = 1
