...
def sim(experiment: Experiment):
    ...
    P_downstate_exc = PoissonGroup(experiment.network_params.C_ext, rates=0.1 * experiment.nu_ext)
    P_downstate_inh = PoissonGroup(experiment.network_params.C_ext, rates=0.1 * experiment.nu_ext)

    S_upstate_exc = Synapses(P_upstate_exc, neurons, model="on: 1", on_pre='g_e += on * g_ampa', method="rk4")
    S_upstate_inh = Synapses(P_upstate_inh, neurons, model="on: 1", on_pre='g_i += on * g_gaba', method="rk4")
    S_downstate_exc = Synapses(P_downstate_exc, neurons, model="on: 1", on_pre='g_e += on * g_ampa', method="rk4")
    S_downstate_inh = Synapses(P_downstate_inh, neurons, model="on: 1", on_pre='g_i += on * g_gaba', method="rk4")
