...
from iteration_5_nmda.network_with_nmda import wang_model, sim_and_plot, translated_model, wang_model_extended, \
...
class SpecialRuns(unittest.TestCase):
    ...
    def test_run_2(self):
        ...
        sim_and_plot(Experiment(simulation).with_property(Experiment.KEY_SELECTED_MODEL, wang_model_extended).with_property(PlotParams.KEY_PANEL, "Synchronous regular? Wang model"))
        plt.close()

        sim_and_plot(Experiment(simulation).with_property(Experiment.KEY_SELECTED_MODEL, translated_model_extended).with_property(PlotParams.KEY_PANEL, "Synchronous regular? Model with NMDA 1/2 Translated"))
        plt.close()
