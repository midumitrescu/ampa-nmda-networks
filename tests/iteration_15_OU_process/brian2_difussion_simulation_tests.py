...
from iteration_15_OU_process.brian_lif_diffusion import Brian2DiffusionSimulation


class Brian2DiffusionSimulationTestCase(unittest.TestCase):

    def test_simulate_one_brian_difussion_process(self):
        lif_config = default_diffusion_lif_config
        seed = 9

        dt = 0.1 * ms
        diffusion_simulation = Brian2DiffusionSimulation(dt=dt, seed=seed, testing=True, lif_config=lif_config)
        ...
        diffusion_simulation.plot_diffusion_approx(v_s=v_s, t=t, spike_times=spike_times,
                              ...
                              exp_label="Brian 2 test")

        self.assertEqual(100_000, len(t))
        means = v_s.mean(axis=1)
        self.assertAlmostEqual(-47.70031595921392, means[0])
        self.assertAlmostEqual(-47.011274168743526, means[1])
